...
with tab1:
    ...
    if st.button(t["calculate"]):
        if None in (premiums, benefits, expenses, coverage_units):
            ...
            def show_csm_chart(csm_total, premiums, benefits, expenses, risk_adj, num_years, discount_rate, coverage_units, assump_deltas, show_tutorial=False):
                years = list(range(1, num_years + 1))
                if coverage_units is None:
                    coverage_units = [1] * num_years

                csm_release, csm_balance = calculate_csm_from_pl(csm_total, discount_rate, coverage_units, premiums, actual_premiums, assump_deltas)
